...
func sendWhatsAppMessage(client *whatsmeow.Client, store *MessageStore, recipient string, message string, mediaPath string) (bool, string) {
	...
	// Persist the outgoing message so it lands in messages.db with is_from_me=true.
	// Without this, anything sent through the API (UI/agent) is delivered but never
	// recorded, so downstream views see the chat as "unanswered" forever.
	if store != nil {
		ownSender := ""
		if client.Store.ID != nil {
			ownSender = client.Store.ID.User
		}
		if storeErr := store.StoreMessage(string(resp.ID), recipientJID.String(), ownSender, message, resp.Timestamp, true, "", "", "", nil, nil, nil, 0); storeErr != nil {
			fmt.Printf("Warning: failed to persist outgoing message: %v\n", storeErr)
		} else {
			// Keep chats.last_message_time fresh without clobbering the saved name.
			store.db.Exec(`UPDATE chats SET last_message_time = ? WHERE jid = ?`, resp.Timestamp, recipientJID.String())
		}
	}
